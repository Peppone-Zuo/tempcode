...
[codeA, codeB] = generate_golay_codes(5);

hx = 0.1*ones(1, 4000);
%hx = hx + 0.1*(rand(1, length(hx))-0.5);
...
A_response = A_response + (rand(1, length(A_response))-0.5);
B_response = B_response + (rand(1, length(A_response))-0.5);;
...
plot(addRes(padLength + length(codeA):end - length(codeA) + 1) / (length(codeA) * 2));
